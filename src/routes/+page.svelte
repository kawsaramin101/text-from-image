<script>
    ...
    let error = "";
    let language = localStorage.getItem("default_language") ?? "auto";
    ...
    async function handleFileUpload(files) {
        ...
        if (files.length > 0) {
            image = files[0];
            if (isFileImageAndAcceptedFormat(image)) {
                if (image) {
                ...
                try {
                    ...
                    addToLocalStorage(ret.data.text);
                } catch (err) {
                    error = err;
                    isProcessing = false;
                }
    ...
    async function addToLocalStorage(content) {
        try {
            const newScan = {
                date: new Date(),
                content: content,
...
<main>
    <div class="center">
        <div>
            ...
            <select name="language" id="language" style="width: 200px;" bind:value={language}>
